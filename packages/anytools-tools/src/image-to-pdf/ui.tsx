...
export function ImageToPdfUi() {
  ...
  const [margin, setMargin] = useState<(typeof MARGINS)[number]>(36);
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label htmlFor="i2p-size">{s.pageSize}</Label>
              <Select
                value={pageSize}
                onValueChange={(v) => {
                  setPageSize(v as PageSizeId);
                  reset();
                }}
              >
                <SelectTrigger id="i2p-size">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="i2p-orientation">{s.orientation}</Label>
              <Select
                value={orientation}
                onValueChange={(v) => {
                  setOrientation(v as OrientationId);
                  reset();
                }}
                disabled={pageSize === 'fit'}
              >
                <SelectTrigger id="i2p-orientation">
                  <SelectValue>{orientationLabel[orientation]}</SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {ORIENTATIONS.map((id) => (
                    <SelectItem key={id} value={id}>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="i2p-margin">{s.margin}</Label>
              <Select
                value={String(margin)}
                onValueChange={(v) => {
                  setMargin(Number(v) as (typeof MARGINS)[number]);
                  reset();
                }}
              >
                <SelectTrigger id="i2p-margin">
                  <SelectValue>{marginLabel[margin]}</SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {MARGINS.map((m) => (
            ...
            <CheckboxField
              label={s.downscale.replace('{dpi}', String(PRINT_DPI))}
