...
There is a **Stop** button, and it terminates the worker immediately rather than waiting for the current page. Use the page range field if you only need part of a document: reading pages 12–14 of a 200-page scan takes seconds.
...
Two reasons, both reported rather than hidden. Words the recogniser was less than 30% confident about are left out on purpose: at that level they are usually speckles read as punctuation, and including them makes selecting a line copy junk. And for English, Spanish and Portuguese the invisible layer is drawn in Helvetica, whose character set is roughly Latin-1; if OCR produces a character outside it, that one word is skipped and the count is shown to you. Vietnamese uses an embedded Unicode font instead and does not have this limit.
