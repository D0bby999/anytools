...
## Is there a length limit on the command?

Yes — 8,000 characters. Beyond that the request is rejected with HTTP 413 rather than parsed. Commands that long are almost always a `--data` payload that would be better read from a file, which the converter cannot resolve anyway.
