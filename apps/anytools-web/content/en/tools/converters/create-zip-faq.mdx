...
Nothing artificial, but two real ones. The tab's memory is the practical ceiling. Beyond that, this writes a standard (non-Zip64) archive, so individual entries and the archive as a whole must stay under 4 GB — an archive past that limit would be silently wrong rather than refused, which is worth knowing before you try.
