...
The one thing to know about the JSON output is that every value is a string, including numbers. A spreadsheet cell carries no JSON type, and guessing would corrupt real data — a part number like `007` becomes `7`, a phone number becomes a float. If you want typed values, take the CSV into the [CSV ↔ JSON converter](/en/converters/csv-json), which infers types while it parses, or into [JSON Formatter](/en/formatters/json-formatter) to check the result. To turn a Word document into text the same way, see [DOCX to Markdown](/en/converters/docx-to-markdown).
