...
The file will very often be **larger** than the HEIC, sometimes twice the size, because JPEG is simply a less efficient codec. That is expected, not a bug. If the size matters, take the result to [Compress Image](/en/image/compress-image), or drop the dimensions with [Resize Image](/en/image/resize-image) first. To collect a set of converted photos into one document, [Image to PDF](/en/pdf/image-to-pdf) takes JPGs a page at a time.
